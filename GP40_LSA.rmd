...
```
Standardization is not needed because the Tumor cellularity is in the same scale in the 3 different groups of the proteomic subtype.
...
# Filter data based on IQR thresholds
filtered_dataset2 <- cancer_dataset %>%
  filter(age >= lower_bound & age <= upper_bound)

# Create boxplot with the filtered data
ggplot(filtered_dataset2, aes(x = Recurr_status, y = age)) +
...
# Create boxplot with the filtered data
ggplot(filtered_dataset, aes(x = Recurr_status, y = `Disease_free_survival_(m)`)) +
...
There are outliers present in the variables. These will be filtered out. 

Combine both of the filtered datasets:
```{r}
# Combine both filtered datasets using an inner join
combined_filtered_dataset <- filtered_dataset2 %>%
  inner_join(filtered_dataset3, by = colnames(cancer_dataset))

# Verify the combined dataset
print(combined_filtered_dataset)
```

Distribution:
```{r}
ggplot(combined_filtered_dataset, aes(x = age)) +
  geom_histogram(binwidth = 5, fill = "red", color = "black") +
  theme_minimal()
```
...
```{r}
library(ggpubr)
ggqqplot(combined_filtered_dataset$age)+
  ggtitle("Disease free survival")
```
P-values testing with Shapiro for normal distribution:
```{r}
combined_filtered_dataset$age <- as.numeric(as.character(combined_filtered_dataset$age))

shapiro.test(combined_filtered_dataset$age)
```
...
```{r}
library(ggpubr)
ggqqplot(combined_filtered_dataset$`Disease_free_survival_(m)`)+
  ggtitle("Disease free survival")


shapiro.test(combined_filtered_dataset$`Disease_free_survival_(m)`)
```
The data for Disease_free_survival_(m) is not normally distributed.

P-values testing with Shapiro for normal distribution:
```{r}
combined_filtered_dataset$age <- as.numeric(as.character(combined_filtered_dataset$`Disease_free_survival_(m)`))
...
Interpretation:
Based on these results, the Disease_free_survival_(m) data in the combined_filtered_dataset does not follow a normal distribution. The significant deviation from normality implies that statistical methods relying on the assumption of normality may not be appropriate for this variable.

Histograms:
```{r}
ggplot(data = combined_filtered_dataset, aes(x = age,
                     fill = Recurr_status))+
  geom_histogram()+
  theme_classic()
```
```{r}
ggplot(data = combined_filtered_dataset, aes(x = `Disease_free_survival_(m)`,
                     fill = Recurr_status))+
  geom_histogram()+
  theme_classic()
```
...
Logistic model:
```{r}
blm_recurr <- glm(Recurr_status ~ age+Gender+`Disease_free_survival_(m)`, data=combined_filtered_dataset, family="binomial" )
...
OR=0.9642 (as per your output).This indicates that for every one-unit increase in Disease-free survival (in months), the odds of recurrence decrease by about 1−0.964 ≈3.6%, holding all other variables constant.
...
surv_object <- Surv(combined_filtered_dataset$`Disease_free_survival_(m)`, as.numeric(combined_filtered_dataset$Recurr_status))

# fit a KM curve to the data
fit <- survfit(surv_object ~ Recurr_status, data=combined_filtered_dataset)
summary(fit)
```
```{r}
# plot this curve
ggsurvplot(fit, data = combined_filtered_dataset,
...
Log rank test:
```{r}
fitdif <- survdiff(surv_object ~ Recurr_status, data=combined_filtered_dataset)
fitdif
```
