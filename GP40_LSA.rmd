...
Hypothese 2:
=> Welke eiwitten hebben invloed op de recurrence status (machine learning)
==> Age, gender and disease free survival effect on recurrence status (Multivariate analysis)
...
```
The data for Disease_free_survival_(m) is not normally distributed. This means the distribution of the variable significantly deviates from a normal distribution.
...
OR=0.9639 (as per your output). If correct, this would indicate:

For every one-unit increase in Disease-free survival (in months), the odds of recurrence decrease by about 1−0.964≈3.6%, holding all other variables constant.
